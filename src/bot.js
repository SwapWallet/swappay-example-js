...
async function start(ctx) {
  ...
  message += 'می‌توانید کد منبع این دموی آزمایشی را در [github.com/SwapWallet](https://github.com/SwapWallet/swappay-example-js) پیدا کنید.'
...
bot.action(/^coin:(?<token>[a-z]+)-(?<network>[a-z]+)$/i, async (ctx) => {
  ...
  const directInvoice = await invoiceApp.getInvoiceWalletAddressFromBackend("1", token, network)
  ...
  let buttons = links.map(link => [Markup.button.url(link.name, link.url)])
